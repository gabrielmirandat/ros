...
: nh_(nh),
  ...
  base_pos_x_(0.0), base_pos_y_(0.0),
...
void Trabalho::odomCallback(const nav_msgs::Odometry::ConstPtr& pose_msg)
{
    ...
    this->pose_msg_.pose.pose.position.x += base_pos_x_;
    this->pose_msg_.pose.pose.position.y += base_pos_y_;

    occ_grid_->odomCallback(this->pose_msg_);
    point_kin_->odomCallback(this->pose_msg_);
}
...
void Trabalho::spinTrab5()
{
    ...
    while(topo_map_->nodeGivenPosition(base_pos_x_, base_pos_y_) == -1)
    {
        std::cout << "Deve estar dentro de um no" << std::endl;
        std::cout << "[double double] Informe a posição inicial: ";  std::cin >> base_pos_x_; std::cin >> base_pos_y_;

        // ROS_ASSERT(topo_map_->nodeGivenPosition(base_pos_x_, base_pos_y_) != -1);
    }
    ...
    while(running)
    {
        ...
        if(explore_flag)
        {
            ...
            // int vv[/*tam_path*/] = {H,J,M,O,R,O,M,L,K,L}; //10
            // int vv[/*tam_path*/] = {J,M,O,R,O,M,L,K,L}; //9
            int vv[/*tam_path*/] = {M,O,R,O,M,L,K,L}; //8
            // int vv[/*tam_path*/] = {O,R,O,M,L,K,L}; //7
            // int vv[/*tam_path*/] = {R,O,M,L,K,L}; //6
            // int vv[/*tam_path*/] = {O,M,L,K,L}; //5
            // int vv[/*tam_path*/] = {M,L,K,L}; //4
            // int vv[/*tam_path*/] = {L,K,L}; //3
            // int vv[tam_path] = {K,L}; //2

            for(size_t i=0; i<tam_path; i++)
